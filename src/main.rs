...
fn get_card_row(card: &Card, row: usize) -> String {
    ...
    let mut top_rank: String = rank.to_string();
    let mut bottom_rank: String = rank.to_string();

    if card.rank != 9 { top_rank = format!("{} ", rank); }
    if card.rank != 9 { bottom_rank = format!(" {}", rank); }
